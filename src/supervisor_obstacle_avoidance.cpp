...
ros::Subscriber state_obstacle_avoidance_sub;
...
void state_obstacle_avoidance_callback(const std_msgs::Int8::ConstPtr &state)
{
    ...
    */
    state_obstacle_avoidance = state->data;
    //std::cout << state_obstacle_avoidance << std::endl;
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "supervisor_obstacle_avoidance");
    ...
    state_obstacle_avoidance_sub = nh.subscribe("/state_obstacle_avoidance", 10, state_obstacle_avoidance_callback);
    ...
    while (ros::ok())
    {
        ...
        if (state_obstacle_avoidance == 0 || state_obstacle_avoidance == 1) {
